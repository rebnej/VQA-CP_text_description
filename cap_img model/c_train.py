...
def train(model, train_loader, eval_loader, num_epochs, output, opt=None, s_epoch=0):
    ...
    for epoch in range(s_epoch, num_epochs):
        ...
        for i, (c_i_ids, c_mask, c_token_type, v_i_ids, v_mask, v_token_type, a, q, q_id, c, v_c) in tqdm(enumerate(train_loader)):
            
            batch_size = c_i_ids.size(0)
            
            c_i_ids = c_i_ids.view(batch_size, -1)
            c_mask = c_mask.view(batch_size, -1)
            c_token_type = c_token_type.view(batch_size, -1)
            
            v_i_ids = v_i_ids.view(batch_size, -1)
            v_mask = v_mask.view(batch_size, -1)
            v_token_type = v_token_type.view(batch_size, -1)
            
            if i == 0:
                print('c_i_ids, v_i_ids : shape : ', c_i_ids.shape, v_i_ids.shape)
                print(q_id, q)
                
            c_i_ids = c_i_ids.cuda()
            c_mask = c_mask.cuda()
            c_token_type = c_token_type.cuda()
            
            v_i_ids = v_i_ids.cuda()
            v_mask = v_mask.cuda()
            v_token_type = v_token_type.cuda()
            
            a = a.cuda() #target/question_id

            pred, c_attention, v_attention = model(c_i_ids, c_mask, c_token_type, v_i_ids, v_mask, v_token_type)
            ...
            total_loss += loss.item() * c_i_ids.size(0)
...
def evaluate(model, dataloader):
    ...
    for i, (c_i_ids, c_mask, c_token_type, v_i_ids, v_mask, v_token_type, a, q, q_id, c, v_c) in enumerate(dataloader):
