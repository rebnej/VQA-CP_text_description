...
#images = load_folder(folder='data/train2014', 'jpg')
def load_folder(folder, suffix):
    imgs = []
    for f in sorted(os.listdir(folder)):
        if f.endswith(suffix): #後方一致(jpgファイルだった場合)
            imgs.append(os.path.join(folder, f))
    return imgs #~.jpgのリスト


#imgids = utils.load_imageid(path_imgs[split] = 'data/train2014')
#jpgファイルは, 'COCO_train2014_000000283611.jpg'みたいな名前で
#ここでは283611の部分だけ取り出してsetに集める(整列済み)
...
#logger = utils.Logger(os.path.join(args.output, 'args.txt')) #'saved_models/ban/args.txt'にloggerを作成
class Logger(object):
